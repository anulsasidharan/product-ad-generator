...
interface ChatInterfaceProps {
  ...
  /** Passed to iterate API as `currentState.parameters.productImageUrl` when set. */
  productImageUrl?: string;
  /** Optional parent hook when the user sends a message (e.g. analytics). */
  onIteration?: (message: string) => void;
  /** Called after each send finishes with the full thread for persistence. */
...
export function ChatInterface({
  ...
  const suggestionChips = useMemo(
    () => ["Make it warmer", "Add a headline", "Try different background"],
    [],
  );

  const send = useCallback(async () => {
    const text = input.trim();
    if (!text || pending || isProcessing) {
      return;
    }
    setInput("");
    ...
    try {
      ...
      setPending(false);
      queueMicrotask(() => {
        onHistoryChange?.(toConversationTurns(messagesRef.current));
      });
    }
  }, [
    generation.id,
  ...
  return (
    <Card className="flex h-[min(640px,70vh)] w-full max-w-md flex-col border-slate-200 shadow-sm">
        ...
        {messages.map((m) => (
          <div
            ...
            className={cn(
              ...
              m.role === "user" && "ml-auto bg-primary-600 text-white",
              m.role === "assistant" && "mr-auto bg-slate-100 text-slate-900",
              m.role === "system" && "mx-auto bg-transparent text-center text-xs text-slate-500",
            )}
          >
            {m.imageUrl && m.role === "assistant" && (
              <div className="relative mb-2 aspect-video w-full max-w-[220px] overflow-hidden rounded-md border bg-white">
            ...
            <p className="mt-1 text-[10px] opacity-70">{formatRelativeTime(m.at)}</p>
          </div>
        ))}
        <div ref={bottomRef} />
        <div className="flex flex-wrap gap-1 pt-2">
          {suggestionChips.map((c) => (
            <button
              key={c}
              type="button"
              className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-xs text-slate-700 hover:border-primary-400"
              disabled={busy}
              onClick={() => setInput(c)}
            >
              {c}
      ...
      <CardFooter className="gap-2 border-t p-3">
        ...
        />
        <Button type="button" size="icon" disabled={busy || !input.trim()} onClick={() => void send()} aria-label="Send">
